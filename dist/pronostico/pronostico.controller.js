"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PronosticoController = void 0;
const common_1 = require("@nestjs/common");
const pronostico_dto_1 = require("./dto/pronostico.dto");
const pronostico_service_1 = require("./pronostico.service");
let PronosticoController = class PronosticoController {
    constructor(pronosticoService) {
        this.pronosticoService = pronosticoService;
    }
    async acreatePost(res, createPronosticoDTO) {
        const pronostico = await this.pronosticoService.createPronostico(createPronosticoDTO);
        return res.status(common_1.HttpStatus.OK).json({
            message: "Pronostico Creado",
            pronostico
        });
    }
    async getPronosticos(res) {
        const pronosticos = await this.pronosticoService.getPronosticos();
        return res.status(common_1.HttpStatus.OK).json({
            pronosticos
        });
    }
    async getEncuentro(res, pronosticoID) {
        const pronostico = await this.pronosticoService.getPronostico(pronosticoID);
        if (!pronostico)
            throw new common_1.NotFoundException("No existe el pronostico");
        return res.status(common_1.HttpStatus.OK).json(pronostico);
    }
};
__decorate([
    (0, common_1.Post)("/create"),
    __param(0, (0, common_1.Res)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, pronostico_dto_1.CreatePronosticoDTO]),
    __metadata("design:returntype", Promise)
], PronosticoController.prototype, "acreatePost", null);
__decorate([
    (0, common_1.Get)("/"),
    __param(0, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], PronosticoController.prototype, "getPronosticos", null);
__decorate([
    (0, common_1.Get)("/:pronosticoID"),
    __param(0, (0, common_1.Res)()),
    __param(1, (0, common_1.Param)("pronosticoID")),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], PronosticoController.prototype, "getEncuentro", null);
PronosticoController = __decorate([
    (0, common_1.Controller)('pronostico'),
    __metadata("design:paramtypes", [pronostico_service_1.PronosticoService])
], PronosticoController);
exports.PronosticoController = PronosticoController;
//# sourceMappingURL=pronostico.controller.js.map